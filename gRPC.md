...
!!! info ""
    ...
    protobuf バイナリ:
      0x08 0x7B 0x12 0x05 "Alice" 0x18 0x1E
      → 約 10〜15 バイト・スキーマ既知なので高速デシリアライズ
    ```
...
!!! info ""
    ...
    値 300 のエンコード:
      ...
      1. 下位 7 ビット: 0101100 = 0x2C → MSB=1: 0xAC（続きあり）
      2. 残り: 0000010 = 0x02 → MSB=0: 0x02（終わり）
      エンコード: [0xAC, 0x02] = 2 バイト
    
      JSON では "300" = 3 バイト（しかも文字列として）
    ```
