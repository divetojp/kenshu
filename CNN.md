...
畳み込みは、フィルタ（カーネル）を入力画像の領域に重ねて積和演算（要素ごとの積の合計）を行い、それを全体に適用して特徴マップを生成します。

フィルタ `[[1,0,-1],[1,0,-1],[1,0,-1]]` は「縦エッジ検出器」で、左辺が明るく右辺が暗い境界に強く反応します。

入力（5×5）の左上 3×3 領域に対してフィルタを適用した場合：

$$1 \times 1 + 0 \times 0 + 1 \times (-1) + 0 \times 1 + 1 \times 0 + 0 \times (-1) + 1 \times 1 + 0 \times 0 + 1 \times (-1) = 0$$

フィルタを 1 ピクセルずつずらしながら同じ計算を繰り返すと、3×3 の出力特徴マップが得られます（パディングなし・ストライド 1 の場合、出力サイズ = 入力サイズ − フィルタサイズ + 1）。
```python
import numpy as np
...
テンソルの形 `(8, 32, 224, 224)` の各次元の意味：

- 第 1 次元（8）— バッチサイズ：一度に処理する画像枚数
- 第 2 次元（32）— チャンネル数：フィルタの種類数（検出する特徴の数）
- 第 3 次元（224）— 高さ H
- 第 4 次元（224）— 幅 W
> **情報工学メモ：なぜ全結合層より良いか**  
