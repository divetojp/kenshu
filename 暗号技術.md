...
sequenceDiagram
    ...
    participant 受信者
    送信者->>受信者: 暗号化(鍵K) → 暗号文
    Note over 送信者,受信者: 送信者・受信者ともに同じ鍵 K を使用
    受信者->>受信者: 復号(鍵K) → 平文
```
### 代表的なアルゴリズム
...
| 鍵の生成 | 安全性の根拠 |
|---|---|
| 大きな素数 p, q を選ぶ（例: 各 2048 bit） | n を素因数分解して p, q を求めることが |
| n = p × q を計算（公開） | 現代のコンピュータでは計算上不可能（2048 bit の場合） |
| 公開鍵: (n, e)、秘密鍵: (n, d) |  |
| 暗号化: C = M^e mod n |  |
| 復号:   M = C^d mod n |  |
```python
...
sequenceDiagram
    ...
    C->>S: "TLS 1.3 使いたい"
    S->>C: サーバの公開鍵証明書（CA が署名済み）
    Note over C: 公開鍵を取り出し、ランダムなセッション鍵（共通鍵）を生成
    C->>S: セッション鍵をサーバの公開鍵で暗号化して送信
    Note over S: 秘密鍵でセッション鍵を復号 → 両者がセッション鍵を共有
    C->>S: セッション鍵（AES）で高速に暗号化通信
    S->>C: セッション鍵（AES）で高速に暗号化通信
```
| フェーズ | 使う暗号 | 目的 |
|--------|---------|------|
