...
Reliability Diagram は、横軸に「モデルの予測確率」、縦軸に「実際の正解率」を取ったグラフです。**完璧なキャリブレーションでは全点が対角線上に並びます**。例えば、「予測確率 0.8」のサンプル群の実際の正解率が 0.8 に近いほどキャリブレーションが良い状態です。点が対角線から上にずれると「過小信頼（actual > predicted）」、下にずれると「過大信頼（過信）」を意味します。
### Temperature Scaling
