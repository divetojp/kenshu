...
| ❌ よくやってしまうこと | ✅ 正しいやり方 |
|---|---|
| 実験中に毎日 p 値を確認して、有意になったら止める | 事前に決めたサンプルサイズに達するまで待つ |
| 「月曜: p=0.08（続行）」「火曜: p=0.04（有意！止めよう）」 | またはシーケンシャルテスト（逐次検定）を使う |
...
| SNS・マーケットプレイス・紹介プログラムなどで起きる問題 | 対処 |
|---|---|
| A群のユーザーが B群のユーザーに影響を与えてしまう | クラスター単位でランダム化（個人でなく地域・グループで割り当て） |
| 例: 「友達紹介機能」の A/B テスト |  |
