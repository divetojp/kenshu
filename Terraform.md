...
| 手動管理の問題 | IaC（Terraform） |
|---|---|
| 開発者A: コンソールでサーバーを作成 | git log で「誰が・いつ・何を」変更したか一目瞭然 |
| 開発者B: 「このサーバーどうやって作ったの？」→ 不明 | terraform plan でデプロイ前に変更内容を確認 |
| 本番環境: 誰かが手動で設定変更 → ドキュメントと乖離 | 同じコードから開発・ステージング・本番を再現 |
| 障害発生: 同じ構成を再現しようとしても手順が不明 | PR レビューでインフラ変更をコードレビューできる |
---
...
`terraform.tfstate` はどのリソースが実際に作成されているかを記録した「現実の状態」ファイルです。`main.tf` が「あるべき状態」の定義にあたり、`terraform apply` を実行するとこの 2 つの差分を計算してクラウド上のリソースへ反映します。
