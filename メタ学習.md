...
| Support set（少量の学習サンプル） | Query set（分類したいサンプル） |
|---|---|
| クラス A: [画像1, 画像2]   ← K=2 サンプル | ? → クラス A か B か C か？ |
| クラス B: [画像3, 画像4] |  |
| クラス C: [画像5, 画像6] |  |
...
| Support set のサンプルを埋め込み | クエリ q の分類 |
|---|---|
| クラス A: [embed(a₁), embed(a₂)] → 平均 → プロトタイプ cA | d(embed(q), cA) = 0.3  ← 最小 → クラス A に分類 |
| クラス B: [embed(b₁), embed(b₂)] → 平均 → プロトタイプ cB | d(embed(q), cB) = 1.2 |
| クラス C: [embed(c₁), embed(c₂)] → 平均 → プロトタイプ cC | d(embed(q), cC) = 0.9 |
**分類確率：**
...
| | 通常の学習 | MAML（メタ学習） |
|--|-----------|---------------|
| 目的 | $\theta$ を特定タスクに最適化 | 「どのタスクでも少数ステップで適応できる初期値 $\theta$」を学習 |
| タスク A | $\theta \to \theta_A$ | $\theta + \alpha\Delta\theta_A \approx$ タスク A で良い性能 |
| タスク B | $\theta \to \theta_B$ | $\theta + \alpha\Delta\theta_B \approx$ タスク B で良い性能 |
| 汎化性 | 特定タスク専用 | 未知タスクにも数ステップで適応 |
### MAML の更新式
