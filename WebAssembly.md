...
```
JavaScript の問題:
  - 動的型付け → 実行時に型推論のオーバーヘッド
  - JIT コンパイル → 初回実行が遅い・最適化に時間がかかる
  - GC がある → 予測不能な停止が起きる

WebAssembly の解決:
  - 静的型付きバイナリ → 型推論不要・ほぼ機械語と同速
  - AOT コンパイル済み → ブラウザが即デコードして実行
  - 手動メモリ管理（線形メモリ）→ GC ポーズなし
```
...
```
Wasm 線形メモリ（ArrayBuffer として JS からも見える）:
  [0x00][0x01][0x02][0x03]...[0xFF]...
        ↑                    ↑
    Wasm が書いた          JS が読む
    画像データ             （SharedArrayBuffer）

注意: JS オブジェクト・DOM へのアクセスは Wasm から直接不可
     wasm-bindgen がブリッジコードを自動生成してくれる
```
...
```
従来:  コード → OS 依存のバイナリ（Windows/Linux/macOS で別々）
WASI:  コード → .wasm（OS に関係なく 1 つのバイナリ）
                ↓
       WASI ランタイム（Wasmtime・WasmEdge）が OS 呼び出しをラップ
```
