...
| | pandas（行指向メモリ） | Polars（列指向メモリ） |
|--|----------------------|----------------------|
| データ配置 | 1 行ごとにまとめて格納 | 列ごとにまとめて格納 |
| 列集計の際 | 全行をスキャンする必要がある | 対象列だけを読めばよい |
| キャッシュ効率 | 低い（無関係なデータも読む） | 高い（必要な列のみ連続アクセス） |
...
| | pandas（即時実行） | Polars Lazy（最適化後に実行） |
|--|-----------------|------------------------------|
| フィルタ | 全データをその場でフィルタ | 後続の select と組み合わせて最適化 |
| 列選択 | フィルタ後の全列を処理 | 使わない列は最初から読み込まない（述語プッシュダウン） |
| 実行タイミング | 各メソッド呼び出しごと | `.collect()` を呼ぶまで遅延 |
---
