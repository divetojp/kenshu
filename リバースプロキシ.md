...
| フォワードプロキシ（クライアント側） | リバースプロキシ（サーバー側） |
|---|---|
| クライアント → [プロキシ] → インターネット | インターネット → [リバースプロキシ] → バックエンドサーバー群 |
| ・クライアントが「どこを見ているか」を隠す | ・サーバーの実態（IP・ポート・台数）を隠す |
| ・企業内のインターネットアクセス制限 | ・負荷分散・SSL 終端・キャッシュ |
| ・VPN の一形態 |  |
---
