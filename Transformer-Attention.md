...
- **Keys（索引）**：「私」「東京」「本」「今日」 … （文中の各単語）
- **Values（内容）**：各単語の意味ベクトル

Query と各 Key の類似度を計算すると「私」が最も一致し、「私」の Value を重点的に取ってくる形になります。
...
同じ入力から複数の「頭（ヘッド）」が異なる重み行列で Q・K・V を生成し、それぞれ独立に Attention を計算します：

- Head 1：構文的な関係（主語・述語など）を学習
- Head 2：意味的な関係（類義語・関連概念）を学習
- Head 3：照応関係（代名詞の指示先）を学習
- …（合計 $h$ 個）

各ヘッドの出力を結合し、線形変換を経て最終出力を得ます。
