...
!!! info ""
    ```
    自然界の進化:
      個体群 → 環境で生き残れる個体が繁殖 → 子孫に形質が遺伝
             → 突然変異で多様性 → 何世代もかけて適応
    
    遺伝的アルゴリズム:
      解の集合（個体群） → 良い解ほど次世代に残る（選択）
      → 2つの解を組み合わせる（交叉） → ランダムに変化（突然変異）
      → 何世代も繰り返して良い解に収束
    ```
...
!!! info ""
    ```
    金属の焼きなまし（物理）:
      高温: 原子が活発に動き回る → エネルギーが低い安定状態を探す
      徐々に冷却: 動きが鈍くなる → 最終的に安定な結晶構造に落ち着く
    
    焼きなまし法:
      高温（初期）: 悪い解でも受け入れる確率が高い → 局所最適から脱出できる
      徐々に冷却: 受け入れ確率が下がる → 良い解の周辺で精密に探索
