...
| エッジに適した用途 | クラウドに適した用途 |
|---|---|
| ・ウェイクワード検出（常時起動・低電力） | ・大規模言語モデル（膨大な計算量） |
| ・自動車の衝突回避（超低レイテンシ） | ・バッチ処理・学習 |
| ・産業機器の異常検知（ネットワーク非依存） | ・精度最優先のタスク |
| ・スマホのカメラAI（プライバシー保護） |  |
---
...
ハードラベルは 1 クラスだけが 1 で情報量が少ないのに対し、教師モデルの出力するソフトラベルはクラス間の類似性を含む豊かな情報を持ちます。

| | ラベル例 | 特徴 |
|---|---|---|
| **ハードラベル**（正解ラベル） | `[0, 0, 1, 0, 0]` | 正解クラスのみ 1、情報量が少ない |
| **ソフトラベル**（教師モデル出力）| `[0.01, 0.02, 0.95, 0.01, 0.01]` | クラス間の類似関係を含む確率分布 |
