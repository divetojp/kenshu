...
| 識別モデル（これまで） | 生成モデル（このページ） |
|---|---|
| 入力: 猫の画像 → モデル → 出力: 「猫」 | 入力: ランダムなノイズ (z) → モデル → 出力: 猫の画像（本物らしい） |
...
| Forward process（データ → ノイズ） | Reverse process（モデルが学習） |
|---|---|
| 猫の画像 → 少しノイズを加える → もっとノイズを加える → ... → 完全なランダムノイズ | ランダムノイズ → 少しノイズを取り除く → ... → 猫の画像 |
