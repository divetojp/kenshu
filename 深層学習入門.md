...
ネットワークの設定：入力 $x = 2.0$、重み $w = 0.5$、バイアス $b = 0.1$、正解ラベル $y_\text{true} = 1.0$

処理の流れ：$x \xrightarrow{\text{線形変換}} z \xrightarrow{\text{sigmoid}} \hat{y} \xrightarrow{\text{MSE}} L$
**■ 順伝播（Forward Pass）**
...
目標：$\partial L/\partial w$（「$w$ を少し増やしたとき、損失はどう変わるか」）を求めます。逆向きの経路 $L \leftarrow \hat{y} \leftarrow z \leftarrow w$ に沿って連鎖律を適用します。

**STEP 2-4: 損失 → $\hat{y}$ への勾配**

$$\frac{\partial L}{\partial \hat{y}} = 2(\hat{y} - y_\text{true}) = 2 \times (0.750 - 1.0) = -0.500$$

**STEP 2-5: $\hat{y}$ → $z$ への勾配（sigmoid の微分）**

$$\frac{\partial \hat{y}}{\partial z} = \sigma(z)(1 - \sigma(z)) = 0.750 \times 0.250 = 0.188$$
...
$$\frac{\partial L}{\partial w} = \frac{\partial L}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial z} \cdot \frac{\partial z}{\partial w} = (-0.500) \times 0.188 \times 2.0 = -0.188$$

$$\frac{\partial L}{\partial b} = \frac{\partial L}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial z} \cdot \frac{\partial z}{\partial b} = (-0.500) \times 0.188 \times 1.0 = -0.094$$
